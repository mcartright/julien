...
public class WikiWexParser extends DocumentStreamParser {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      if (!filter(data)) {
        ...
        if (data.length > 11) {
          ...
          externalInlinkCount =
	      parseStanfordLinkData("stanf_anchor",
				    stanfLinks,
				    documentText,
				    true);
          parseStanfordLinkData("stanf_anchor-exact",
				stanfLinks,
				documentText,
				false);
        }
        ...
        Document res =
	    new Document(cleanTitle(wikiTitle), documentText.toString());
	    ...
	    .log(Level.WARNING,
		 "Error Line: "
		 + line.substring(0, minLen)
		 + " num fields: " + data.length);
      }
    }
    return null;
  }
  ...
  private boolean filter(String[] data) {
	...
	wikiTitle.startsWith("Category:") ||
	wikiTitle.startsWith("Template:") ||
	wikiTitle.startsWith("List of") ||
	wikiTitle.startsWith("Lists of")) {
      return true;
    }
  ...
  private String cleanText(String rawText) {
    ...
    if (containsBrackets) {
      ...
      while (curIndex < newText.length()) {
        ...
        if (endBracket == -1) {
          ...
          if (endBracket == -1) {
            System.out.println("No ending markup tag backets in String: "
			       + newText);
            break;
          } else {
    ...
    private int parseInlinks(String fieldName,
			     String anchors,
			     StringBuilder sb,
			     boolean tokenize) {
    if (anchors == null || anchors.length() == 0) {
      ...
      while ((curIdx = anchors.indexOf(startText, curIdx)) > -1) {
        ...
        String anchorText =
	    anchors.substring(curIdx + startText.length(), endTxtIdx);

        int startCntIdx = anchors.indexOf(startCnt, curIdx);
        int endCntIdx = anchors.indexOf(endCnt, startCntIdx);
        String countString =
	    anchors.substring(startCntIdx + startCnt.length(), endCntIdx);
  ...
  private int parseStanfordLinkData(String fieldName,
				    String anchors,
				    StringBuilder sb,
				    boolean tokenize) {
    if (anchors == null || anchors.length() == 0) {
      ...
      while ((curIdx = anchors.indexOf(startText, curIdx)) > -1) {
        ...
        String scoreString =
	    anchors.substring(startCntIdx + startScores.length(), endCntIdx);
        ...
        if (!scoreString.equals("null")) {
          ...
          for (String score : scores) {
            ...
            if (externalIdx > -1) {
              ...
              String numerator =
		  score.substring(externalIdx + startExternal.length(),
				  dividerIdx);
  ...
  private String cleanAnchors(String text) {
    String[] termsToReplace = {"wikipedia the free encyclopedia",
  ...
  private boolean filterAnchorText(String text, int count) {
    ...
    Set<String> wikiStopwordSet =
	new HashSet<String>(Arrays.asList(wikiStopwords));
    if (text.length() == 0 ||
	(text.startsWith("[") && text.endsWith("]")) ||
	wikiStopwordSet.contains(text.toLowerCase())) {
      return true;
    } else {
      return false;
    }
  ...
  private Iterable<String> parseSources(String sources) {
    if (sources == null || sources.length() == 0) {
      ...
      while ((curIdx = sources.indexOf(startText, curIdx)) > -1) {
        ...
        String source =
	    sources.substring(curIdx + startText.length(), endTxtIdx);
